...
const  baseUrl = "http://localhost:8080/api/v1/storages"
...
const addStorageToPc = async (storageId, pc_Id) =>{
    const response = await fetch(`${baseUrl}/${storageId}/${pc_Id}`, {
        method: 'POST',
        headers: headers
    })
        .catch(err => console.error(err.message))
...
const createProductCards = (arr) =>{
    ...
    arr.forEach(obj =>{
        ...
        <div class="card-body d-flex flex-column justify-content-between">
            ...
            <div>
                <button onclick="addStorageToPc(${obj.id}, pcId)" class="btn btn-primary">Add To Pc</button>
